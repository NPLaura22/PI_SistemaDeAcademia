...
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'src_frontend', 'login2.html'));
});
    ...
    const query = `
        SELECT 
            ...
            SEC_TO_TIME(SUM(TIME_TO_SEC(CASE 
                WHEN WEEK(C.HORARIO_ENTRADA) = WEEK(CURDATE()) AND YEAR(HORARIO_ENTRADA) = YEAR(CURDATE()) THEN C.TOTAL_HORAS
                ELSE '00:00:00'
            END))) AS total_horas_semanal,
            SEC_TO_TIME(SUM(TIME_TO_SEC(C.TOTAL_HORAS))) AS total_horas_geral,
            CASE
                WHEN SUM(TIME_TO_SEC(CASE WHEN WEEK(C.HORARIO_ENTRADA) = WEEK(CURDATE()) AND YEAR(C.HORARIO_ENTRADA) = YEAR(CURDATE()) THEN C.TOTAL_HORAS ELSE '00:00:00' END)) <= 5 * 3600 THEN 'Iniciante'
                WHEN SUM(TIME_TO_SEC(CASE WHEN WEEK(C.HORARIO_ENTRADA) = WEEK(CURDATE()) AND YEAR(C.HORARIO_ENTRADA) = YEAR(CURDATE()) THEN C.TOTAL_HORAS ELSE '00:00:00' END)) BETWEEN 6 * 3600 AND 10 * 3600 THEN 'Intermediário'
                WHEN SUM(TIME_TO_SEC(CASE WHEN WEEK(C.HORARIO_ENTRADA) = WEEK(CURDATE()) AND YEAR(C.HORARIO_ENTRADA) = YEAR(CURDATE()) THEN C.TOTAL_HORAS ELSE '00:00:00' END)) BETWEEN 11 * 3600 AND 20 * 3600 THEN 'Avançado'
                WHEN SUM(TIME_TO_SEC(CASE WHEN WEEK(C.HORARIO_ENTRADA) = WEEK(CURDATE()) AND YEAR(C.HORARIO_ENTRADA) = YEAR(CURDATE()) THEN C.TOTAL_HORAS ELSE '00:00:00' END)) > 20 * 3600 THEN 'Extremamente avançado'
